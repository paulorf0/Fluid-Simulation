...
bool Circle::check_collision(Particle *p)
{
    ...
    sf::Vector2f c_pos = this->get_pos(); 
...
{
    sf::Vector2f dif = p->get_pos() - pos;
    float r2 = dif.x * dif.x + dif.y * dif.y;
    
    if (r2 < 0.0001f) return sf::Vector2f(0, -1); 
    float r = sqrt(r2);
    return dif / r;
}
...
void Circle::extractParticle(Particle *p)
{
    if(check_collision(p)) {
        ...
        if (r2 < 0.0001f) {
            p->set_pos(pos + sf::Vector2f(0, -(radius + p->get_radius())));
            return;
        }
